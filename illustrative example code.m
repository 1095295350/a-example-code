...
%单位kw光伏板的功率(Power per kW of photovoltaic panels)
datapv=[0.00 
0.00 
0.00 
0.00 
...
%每小时用电需求GW(Hourly electricity demand GW)
data1=[22.4488704
21.32642688
20.20398336
20.76520512
...
%购电电价(Electricity purchase price)
data2=[0.2289
0.2289
0.2289
0.2289
...
%改24小时用电排列(24-hour electricity arrangement)
...
%安装光伏，燃气，储能的数量(Quantity of installed photovoltaic, gas, and energy storage)
pvn=intvar(1,1,'full');
gtn=intvar(1,1,'full');
ban=intvar(1,1,'full');

%购电的数量(Quantity of electricity purchased)
ge=sdpvar(k,1,'full');
%燃气轮机开启产生的发电量(Power generation generated by gas turbine start-up)
gtz=sdpvar(k,1,'full');
gtz(1,1)=0;
gtz(25,1)=0;
gtz(49,1)=0;
gtz(73,1)=0;
%每日光伏发电量(Daily photovoltaic power generation)
pvz=sdpvar(k,1,'full');

%实际清洁能源比例(Actual clean energy ratio)
ac=(sum(pvz)+sum(gtz))/(sum(ge)+sum(pvz)+sum(gtz));
%光伏发电总量(Total photovoltaic power generation)
spvz=sum(pvz);
%燃气发电总量(Total Gas-fired power plant)
sgtz=sum(gtz);
%买电总量(Total electricity purchased)
sge=sum(ge);

%定义弹性指标(Define resilience)
R1=sdpvar(24,1,'full');
R2=sdpvar(24,1,'full');
R3=sdpvar(24,1,'full');
R4=sdpvar(24,1,'full');

%能完全恢复为1，不能为0（Can fully recover to 1, cannot be 0）
ce1=sdpvar(24,1,'full');
ce2=sdpvar(24,1,'full');
ce3=sdpvar(24,1,'full');
ce4=sdpvar(24,1,'full');

%通过0-1变量实现条件语句（Implement conditional statements through 0-1 variables）
d1=binvar(24,2,'full');
d2=binvar(24,2,'full');
d3=binvar(24,2,'full');
d4=binvar(24,2,'full');
...
%目标函数1：经济成本   （单位元）  （Objective function 1: economic cost ）
 f1=815.14*1000*pvn+1091*1000*gtn+215.11*1000*ban+sum(gtz)*0.1975*2.86*91250+sum(ge'*data2)*91250;
 %目标函数2：碳排放 kg （Objective Function 2: Carbon Emission ）
 f2=(sum(gtz)*0.4699+sum(ge)*0.8*0.8478)*91250;
 %目标函数3：恢复力 （Objective Function 3: Resilience ）
...
%燃气轮机爬坡速率(up，down)  （Power generation of gas turbine is less than Nameplate capacity）
for i=1:23
...
%购电（Electricity purchase）
for i=1:k
    F=[F,-ge(i,1)<=0];
end

%每小时光伏发电（Hourly photovoltaic power generation）
for i=1:k
    F=[F,pvz(i,1)==pvn*datapv(i,1)];
end

%电池充放电(要么充要么放)（Battery charging and discharging (either charging or discharging)）
for i = 1:96
    F = [F;sum(d5(i,:)) == 1;
...
%定义充放电的上下限(按照最大三小时充放完)（Define the upper and lower limits for charging and discharging (based on a maximum of three hours of charging and discharging)）
for i=1:96
...
%定义电池状态最大最小（Define the maximum and minimum battery status）
for i=1:96
...
%每小时弹性说明（Explanation of hourly resilience）
for i=1:24
...
%判断是否能完全恢复电力（Determine if power can be fully restored）
for i = 1:24
    F = [F;sum(d1(i,:)) == 1;
...
%目标2碳排放改约束（Objective 3 Carbon emission reform constraints）
F=[F,(sum(gtz)*0.4699+sum(ge)*0.8*0.8478)*91250<=a*0.8*0.8478*288000000];

 %目标3恢复力改约束（Objective 2 Resilience reform constraints）
F=[F,b*96-sum(ce1)-sum(ce2)-sum(ce3)-sum(ce4)<=0];


%数量安装限制（Quantity installation limit）
F=[F,-pvn<=0];
F=[F,-70+pvn<=0];
...
%满足当局分配 （Meet the allocation of authorities）   
